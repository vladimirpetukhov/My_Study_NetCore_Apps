...
namespace MUSACA.Controllers {
    public class UsersController : BaseController {
        ...
        public IHttpResponse Profile() {
            ...
            var receipts = new UserReceiptsViewModel();

            receipts = this.Db.Users
                .Where(r => r.Username == username)
                .Select(user => new UserReceiptsViewModel {
                    Cashier = user.Username,
                    Receipts = user.Receipts.Select(receip => new UserProfileViewModel {
                        Id = receip.Id,
                        IssuedOn = receip.IssuedOn,
                        Total = receip.Orders.Select(t => t.Total).Sum()
                    }).ToList()
                }).SingleOrDefault();
